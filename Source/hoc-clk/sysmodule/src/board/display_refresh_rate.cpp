...
namespace display {
    ...
    */
    ...
    void CorrectOledGamma(uint32_t refresh_rate) {
        ...
        if (g_config.isDocked || refresh_rate < 43 || refresh_rate > 60) {
        ...
        if (refresh_rate == last_refresh_rate) return;

        // Use existing known-good anchor points (45/50/55/60) and extend to every Hz
        // via smooth linear interpolation in-between, then clamp to panel-safe register ranges.
        static constexpr uint32_t offsets[] = {0x1A, 0x24, 0x25};
        uint32_t values[] = {2, 0, 0x83}; // 60 Hz baseline

        auto lerp_u32 = [](uint32_t a, uint32_t b, float t) -> uint32_t {
            return (uint32_t)lroundf((float)a + ((float)b - (float)a) * t);
        };

        if (refresh_rate <= 45) {
            values[0] = 4; values[1] = 1; values[2] = 0;
        } else if (refresh_rate < 50) {
            const float t = (float)(refresh_rate - 45) / 5.0f;
            values[0] = lerp_u32(4, 3, t);
            values[1] = 1;
            values[2] = 0;
        } else if (refresh_rate < 55) {
            values[0] = 3; values[1] = 1; values[2] = 0;
        } else if (refresh_rate < 60) {
            const float t = (float)(refresh_rate - 55) / 5.0f;
            values[0] = lerp_u32(3, 2, t);
            values[1] = lerp_u32(1, 0, t);
            values[2] = lerp_u32(0, 0x83, t);
        } else {
            values[0] = 2; values[1] = 0; values[2] = 0x83;
        }

        values[0] = std::min<uint32_t>(values[0], 0xFFu);
        values[1] = std::min<uint32_t>(values[1], 0xFFu);
        values[2] = std::min<uint32_t>(values[2], 0xFFu);
