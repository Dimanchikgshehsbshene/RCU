...
namespace config {
    ...
    namespace {
        ...
        int BrowseIniFunc(const char* section, const char* key, const char* value, void* userdata) {
            ...
            if (!strcmp(section, CONFIG_VAL_SECTION)) {
                for (unsigned int kval = 0; kval < HocClkConfigValue_EnumMax; kval++) {
                    if (!strcmp(key, rclkFormatConfigValue((HocClkConfigValue)kval, false))) {
                        input = strtoul(value, NULL, 0);
                        if (!rclkValidConfigValue((HocClkConfigValue)kval, input)) {
                            input = rclkDefaultConfigValue((HocClkConfigValue)kval);
        ...
        void Close() {
            ...
            for (unsigned int i = 0; i < HocClkConfigValue_EnumMax; i++) {
                configValues[i] = rclkDefaultConfigValue((HocClkConfigValue)i);
            }
        }
    ...
    bool SetConfigValues(HocClkConfigValueList* configValues, bool immediate) {
        ...
        for (unsigned int kval = 0; kval < HocClkConfigValue_EnumMax; kval++) {
            if (!rclkValidConfigValue((HocClkConfigValue)kval, configValues->values[kval]) ||
               configValues->values[kval] == rclkDefaultConfigValue((HocClkConfigValue)kval)) {
                continue;
            }
            iniValues.push_back(std::to_string(configValues->values[kval]));
            iniKeys.push_back(rclkFormatConfigValue((HocClkConfigValue)kval, false));
        }
        ...
        if (immediate) {
            for (unsigned int kval = 0; kval < HocClkConfigValue_EnumMax; kval++) {
                if (rclkValidConfigValue((HocClkConfigValue)kval, configValues->values[kval])) {
                    ...
                    config::configValues[kval] = rclkDefaultConfigValue((HocClkConfigValue)kval);
                }
            }
        }
    ...
    bool ResetConfigValue(HocClkConfigValue kval) {
        if (!RCLK_ENUM_VALID(HocClkConfigValue, kval)) {
