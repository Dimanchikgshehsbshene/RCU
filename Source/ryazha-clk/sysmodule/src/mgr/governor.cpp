...
namespace governor {
    ...
    void HandleGovernor(uint32_t targetHz) {
        ...
        if (!tempTargetHz) {
            ...
            if (!tempTargetHz)
                tempTargetHz = config::GetAutoClockHz(RCLK_GLOBAL_PROFILE_TID, HocClkModule_Governor, clockManager::gContext.profile, true);
        }
                ...
                if (!skipVrr) {
                    ...
                    if (fps != 254) {
                        ...
                        if (!targetHz) {
                            ...
                            if (!targetHz)
                                targetHz = config::GetAutoClockHz(RCLK_GLOBAL_PROFILE_TID, HocClkModule_Display, clockManager::gContext.profile, false);
                        }
