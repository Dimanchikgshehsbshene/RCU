...
namespace governor {
    ...
    void CpuGovernorThread(void* arg)
    {
        ...
        for (;;) {
            ...
            if ((!goingDown || (downHoldRemaining == 0)) && clockManager::IsAssignableHz(RClkModule_CPU, newHz)) {
    ...
    void GovernorThread(void* arg)
    {
        ...
        for (;;) {
            ...
            if ((!goingDown || (downHoldRemaining == 0)) && clockManager::IsAssignableHz(RClkModule_GPU, newHz)) {
    ...
    void VRRThread(void* arg)
    {
        ...
        u8 tick = 0, tick2 = 0;
        for (;;) {
            if (!clockManager::gRunning || clockManager::gContext.profile == RClkProfile_Docked || !isVRREnabled) {
                svcSleepThread(POLL_NS);
                continue;
            }
